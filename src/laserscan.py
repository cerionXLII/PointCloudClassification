...
class LaserScan:
    ...
    def set_points(self, points, remissions=None):
        ...
        if self.rot:
            ...
            self.points = self.points @ R.random().as_matrix().T
...
class DesnowedLaserScan(LaserScan):
    snow_indices = None
    pred_file = None

    def add_snow_preds(self, pred_file, snow_label=1):
        self.pred_file = pred_file
        if pred_file.endswith(".pred"):
            preds = np.fromfile(pred_file, dtype=np.int64)
        else:
            preds = np.fromfile(pred_file, dtype=np.int32)
        preds = preds.reshape(-1)
        self.snow_indices = np.where(preds == snow_label)[0]
    ...
    def open_scan(self, filename):
        ...
        scan = np.fromfile(filename, dtype=np.float32)
        scan = scan.reshape((-1, 4))
        scan = np.delete(scan, obj=self.snow_indices.tolist(), axis=0)
        # put in attribute
        points = scan[:, 0:3]  # get xyz
        remissions = scan[:, 3]  # get remission
        if self.drop_points is not False:
...
class SemLaserScan(LaserScan):
    ...
    def open_label(self, filename):
        ...
        if self.drop_points is not False:
            ...
            # self.points = np.delete(self.points, self.points_to_drop)
        # set it
...
class DesnowedSemLaserScan(DesnowedLaserScan):
    """Class that contains LaserScan with x,y,z,r,sem_label,sem_color_label,inst_label,inst_color_label"""
    EXTENSIONS_LABEL = ['.label']

    def __init__(self, sem_color_dict=None, project=False, H=64, W=1024, fov_up=3.0, fov_down=-25.0, max_classes=300,
                 DA=False, flip_sign=False, drop_points=False):
        super(DesnowedSemLaserScan, self).__init__(project, H, W, fov_up, fov_down, DA=DA, flip_sign=flip_sign,
    ...
    def reset(self):
        """ Reset scan members. """
        super(DesnowedSemLaserScan, self).reset()
    ...
    def open_label(self, filename):
        ...
        label = np.fromfile(filename, dtype=np.int32)
        label = label.reshape((-1))
        label = np.delete(label, obj=self.snow_indices.tolist(), axis=0)
        if self.drop_points is not False:
            label = np.delete(label, self.points_to_drop)
            # self.points = np.delete(self.points, self.points_to_drop)
        # set it
